...
export class LevelSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Контейнер для уровней с прокруткой
  ...
  createLevelCard(level, unlocked, stars, price, canBuy, isBoss, y) {
    ...
    // Фон карточки
    const bg = this.add.rectangle(0, 0, w - 40, 72, 0x1a1a3a, 0.85)
    ...
    // Иконка уровня (босс или обычный)
    ...
    scrollZone.on('pointerup', () => {
      ...
      if (Math.abs(this.scrollVelocity) > 1) {
        this.tweens.add({
          ...
          y: this.levelContainer.y + this.scrollVelocity * 5,
          duration: 500,
          ease: 'Power2.easeOut',
          onUpdate: () => {
            const minY = startY - this.totalHeight + scrollHeight;
  ...
  createScrollIndicator(startY, scrollHeight) {
    const w = this.scale.width;
    const indicatorHeight = (scrollHeight / this.totalHeight) * scrollHeight;
    ...
    this.events.on('update', () => {
      const scrollPercent = (this.levelContainer.y - startY) / (startY - this.totalHeight + scrollHeight - startY);
      const indicatorY = startY + 10 + (scrollHeight - 20 - indicatorHeight) * scrollPercent;
