...
export class PlayScene extends Phaser.Scene {
  ...
  // ===== НОВЫЙ МЕТОД: проверка завершения уровня =====
  checkLevelComplete() {
    if (!this.started || this.dead) return;
  ...
  completeLevel() {
    ...
    if (this.collectedCoins >= this.worldConfig.goalCoins) {
      stars = Math.min(3, stars + 1);
    }
    ...
    // Разблокируем следующий уровень в этом мире
    if (this.level < 4) {
